...
class BinaryTree(object):
    ...
    def __eq__(self, other):
        if not isinstance(other, BinaryTree):
            return False
        return self.label == other.label and self.left == other.left and\
    ...
    def get_leaves(self):
        """
        Return the list of leaves present in the tree.
    ...
    def get_children(self):
        """
        Return an iterable of direct children. Empty iterable if no children
        """
        if self.right:
    ...
    BinaryTree("-", BinaryTree(5), BinaryTree(4))).get_ndeep_tree(1)
        ...
        if n < 0:
            ...
            n -= 1
            if self.right:
                return [x for x in self.left.get_ndeep_tree(n)] +\
                       [x for x in self.right.get_ndeep_tree(n)]
            elif self.left:
                return [x for x in self.left.get_ndeep_tree(n)]
            else:
                return []
...
class Tree(object):
    ...
    def create_binary_tree(self):
        ...
        if not self.children:
            return BinaryTree(self.label)
        elif len(self.children) == 1:
            return BinaryTree(self.label,
                left=self.children[0].create_binary_tree())
        elif len(self.children) == 2:
            return BinaryTree(self.label,
                left=self.children[0].create_binary_tree(), right=self.children[1].create_binary_tree())
        else:
            return BinaryTree(self.label,
                left=self.children[0].create_binary_tree(),
                right=Tree(self.label, *self.children[1:]).create_binary_tree())
...
### Exercise 4
# Implement get_leaves method. The method returns the list of all leaves.
...
### Exercise 8
# Check now the class Tree. It is like BinaryTree but it allows any number of
# children.
...
# the leftmost children is kept; the following children become a new node, with
